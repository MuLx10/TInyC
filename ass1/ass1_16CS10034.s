...
.LC1:							                       # Label of string for scanf
	.string	"%ld"
	.align 8
.LC2:							                       # Label for string for 2nd printf
	.string	"\nPI: %10.8lf (using Infinite Series)"
	.align 8
.LC3:							                       # Label for string for 3rd printf
...
######################################################################################
	                       							   # int main()
	.globl	main				                       # main is a global 
	.type	main, @function		                       # main is a function
main:							                       # start main: 

.LFB2:
	.cfi_startproc				                       # Call Frame Information
	pushq	%rbp				                       # Save old bp (base pointer)
	...
	xorl	%eax, %eax			                       # eax = 0 (XOR makes the regiter value zero)
	...
	##################################################################################
	                                                   # # scanf("%ld",&num);
	leaq	-32(%rbp), %rax		                       # rax = (rbp - 32) To store the sum, allocating space
	...
	movq	%rax, %rdi			                       # rdi = rax (argument "num" to be passed to the function.)
	call	monteCarlo 			                       # call monteCarlo(num)
	movq	%xmm0, %rax			                       # rax = xmm0 (storing return value from monteCarlo(num) to rax)
	movq	%rax, -24(%rbp)		                       # (rbp - 24) = rax (Storing the rax value, i.e, return value piD1 from function in stack)
	...
	movq	%rax, -16(%rbp)		                       # (rbp - 16) = rax (Storing the rax value, i.e, return value piD2 from function in stack)

	##################################################################################
	                                                   # # printf("\nPI: %10.8lf (using Infinite Series)",piD2);
	movq	-16(%rbp), %rax		                       # rax = (rbp - 16) (retrieving the return value piD2 from the stack)
	movq	%rax, -40(%rbp)		                       # (rbp - 40) = rax (storing the piD2 value in another memory location in the stack)
	movsd	-40(%rbp), %xmm0	                       # xmm0 = (double)(rbp - 40) (piD2 value is converted to double and stored in xmm0)
	...
	movq	%rax, -40(%rbp)		                       # (rbp - 40) = rax (Storing the piD1 value in another memory location in the stack)
	movsd	-40(%rbp), %xmm0	                       # xmm0 = (double)(rbp - 40) (piD1 value is converted to double and stored in xmm0)
	movl	$.LC3, %edi			                       # load .LC3 string in edi (first argument)
	movl	$1, %eax			                       # eax = 1 (vector register is set to 1 to print 1 floating point data is printed.)
	call	printf				                       # call printf
...
######################################################################################
	...
	.globl	iSeries 			                       # iSeries is a global 
	.type	iSeries, @function	                       # iSeries is a described as function
...
.L11:
	...
	##################################################################################
	                                                    # # i <= n;
	cmpq	-40(%rbp), %rax		                        # comparing i with n ,exit condition for "for loop"
	jle	.L14					                        # jump to .L14 if (i<=n)

	##################################################################################
	                                                    # # return (double)count/n*4.0; if condition does not hold
	pxor	%xmm0, %xmm0		                        # set xmm0 to 0 by XOR
